...
guardSuccess :: Monad m => ListT m (Result x s e a) -> m (Maybe (ListT m (Result x s e a)))
guardSuccess l = do
  (ls, m) <- findFirst (\r -> case r of { ResultError {} -> Left r; ResultSuccess {} -> Right r }) l
  case m of
